...
import { ANCHORS, getNodeTypeDef, migrateAnchor, resolveNodeTypeId } from "./constants";
...
export function normalizeCanvasData(raw) {
  ...
  return {
    ...
    nodes: Array.isArray(data.nodes)
      ? data.nodes.map((n) => {
          const type = resolveNodeTypeId(n.type || "rectangle");
          ...
          return {
            ...
            x: Number(n.x) || 0,
            y: Number(n.y) || 0,
            width: Number(n.width) || def.width,
            height: Number(n.height) || def.height,
          };
        })
      : [],
...
export function getCanvasCenter(scrollEl, zoom = 1) {
  if (!scrollEl) return { x: 400, y: 300 };
  const z = zoom || 1;
  return {
    x: (scrollEl.scrollLeft + scrollEl.clientWidth / 2) / z - 100,
    y: (scrollEl.scrollTop + scrollEl.clientHeight / 2) / z - 44,
  };
}
